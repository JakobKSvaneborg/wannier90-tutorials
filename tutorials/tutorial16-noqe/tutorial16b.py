"""
Tutorial 16b: Verifying localization from a scrambled initial guess (Silicon/BoltzWann)
======================================================================================

Companion to tutorial 16 (no-QE variant). Silicon with 8 sp3 Wannier functions
extracted from 12 bands (disentanglement), using the BoltzWann tutorial data.
We apply a k-dependent random unitary transformation and verify that the
optimizer recovers the same minimum.

This is the same physical system as tutorial 03 (silicon, sp3) but uses a
different DFT calculation (4x4x4 k-mesh) from the BoltzWann tutorial.
"""
...
seed = tutorial_dir / 'Si'

print("=" * 65)
print("  Tutorial 16b: Localization from a scrambled initial guess")
print("  System: Silicon (BoltzWann), 8 sp3 WFs from 12 bands")
print("=" * 65)
print()
...
try:
    ...
    ax.set_title('Tutorial 16b: Silicon (BoltzWann) \u2014 Convergence from good vs scrambled start')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()

    plot_path = tutorial_dir / 'tutorial16b_convergence.png'
